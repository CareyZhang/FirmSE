...
#        rm ${WORK_DIR}/result
    ...
    # build kernel
    # ================================
    if [ ! -e "${WORK_DIR}/buildKernel" ] || [ ! `cat "${WORK_DIR}/buildKernel"` = "true" ]; then
        echo "[*] ${IID} build kernel!!!"
        ${SCRIPT_DIR}/buildKernel.sh ${IID}
    fi
    ...
    if (! egrep -sqi "true" ${WORK_DIR}/web); then
        ...
        # TIMEOUT is set in "firmse.config". This TIMEOUT is used for initial
        # log collection.
        TIMEOUT=$TIMEOUT FIRMSE_NETWORK=${FIRMSE_NETWORK} \
    ...
    # MD handling
    # ================================
    if [ -e ${WORK_DIR}/md_list ]; then
        mkdir -p ${WORK_DIR}/md_binary/log/
        ${SCRIPT_DIR}/buildKM.sh ${IID}

        ${SCRIPT_DIR}/mount.sh ${IID}
        ${SCRIPT_DIR}/getMDBinary.sh ${IID}
        ${SCRIPT_DIR}/umount.sh ${IID}

        ${SCRIPT_DIR}/inferMD.sh ${IID}

        ${SCRIPT_DIR}/mount.sh ${IID}
        cp ${WORK_DIR}/md_list ${IMAGE_DIR}/firmadyne/md_list
        cp -r ${WORK_DIR}/file_ko ${IMAGE_DIR}/firmadyne
        cp -r ${WORK_DIR}/md_ko ${IMAGE_DIR}/firmadyne
        cp -r ${WORK_DIR}/md_config ${IMAGE_DIR}/firmadyne
        ${SCRIPT_DIR}/umount.sh ${IID}

        ${SCRIPT_DIR}/test_emulation.sh ${IID} ${ARCH}
    fi
    ...
    if [ ${OPTION} = "analyze" ]; then
        ...
        # boot debug mode
        # ================================
        BOOT_KERNEL_PATH=`get_kernel ${ARCH} true`
