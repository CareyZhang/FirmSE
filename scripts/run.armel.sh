...
if [ ${QEMU_MACHINE} == *"vexpress-a9"* ]; then
  QEMU_NETWORK="-net nic,model=lan9118,vlan=0 -net user,vlan=0,script=no"
fi

if [ ${QEMU_MACHINE} == *"vexpress-a9"* ]; then
  QEMU_AUDIO_DRV=none qemu-system-arm -m 256 -M ${QEMU_MACHINE} -kernel ${KERNEL} -drive if=sd,driver=file,filename=${IMAGE} -append "firmadyne.syscall=1 root=${QEMU_ROOTFS} console=ttyS0 nandsim.parts=64,64,64,64,64,64,64,64,64,64 ${QEMU_INIT} rw debug ignore_loglevel print-fatal-signals=1 FIRMSE_NETWORK=${FIRMSE_NETWORK} FIRMSE_NVRAM=${FIRMSE_NVRAM} FIRMSE_KERNEL=${FIRMSE_KERNEL} FIRMSE_ETC=${FIRMSE_ETC} user_debug=31" -serial file:${WORK_DIR}/qemu.initial.serial.log -serial unix:/tmp/qemu.${IID}.S1,server,nowait -monitor unix:/tmp/qemu.${IID},server,nowait -display none ${QEMU_NETWORK}
else
