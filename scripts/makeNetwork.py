...
def mountImage(targetDir):
    loopFile = subprocess.check_output(['bash', '-c', 'source firmse.config && add_partition %s/image.raw' % targetDir]).decode().strip()
...
def umountImage(targetDir, loopFile):
    ...
    subprocess.check_output(['bash', '-c', 'source firmse.config && del_partition %s' % loopFile.rsplit('p', 1)[0]])
...
def getWorkDir():
    if os.path.isfile("./firmse.config"):
        return os.getcwd()
    elif os.path.isfile("../firmse.config"):
        path = os.getcwd()
...
def main():
    ...
    workDir = getWorkDir()
    if not workDir:
        raise Exception("Can't find firmse.config file")
